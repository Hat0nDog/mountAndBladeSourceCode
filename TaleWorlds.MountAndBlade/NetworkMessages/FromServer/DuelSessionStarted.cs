...
namespace NetworkMessages.FromServer
{
  ...
  internal sealed class DuelSessionStarted : GameNetworkMessage
  {
    public Agent RequesterAgent { get; private set; }

    public Agent RequestedAgent { get; private set; }

    public DuelSessionStarted(Agent requesterAgent, Agent requestedAgent)
    {
    ...
    protected override string OnGetLogFormat() => "Duel session started between agent with name: " + this.RequestedAgent.Name + " and index: " + (object) this.RequestedAgent.Index + " and agent with name: " + this.RequesterAgent.Name + " and index: " + (object) this.RequesterAgent.Index;
  }
}
